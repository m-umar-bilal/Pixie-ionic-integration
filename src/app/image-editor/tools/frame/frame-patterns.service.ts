...
import {Image as IImage} from 'fabric/fabric-impl';
...
export class FramePatternsService {
    ...
    private scalePatternToWidth(pattern: IImage, value: number) {
        let boundingRectFactor = pattern.getBoundingRect().width / pattern.getScaledWidth();
    ...
    private scalePatternToHeight(pattern: IImage, value: number) {
        let boundingRectFactor = pattern.getBoundingRect().height / pattern.getScaledHeight();
    ...
    public load(frame: Frame) {
        const promises = this.activeFrame.getPartNames().map(part => {
            return new Promise(resolve => {
                const img = new Image();
                img.onload = () => {resolve({name: part, img: new fabric.Image(img, staticObjectConfig)})};
