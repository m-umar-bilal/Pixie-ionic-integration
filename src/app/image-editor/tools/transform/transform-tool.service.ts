...
export class TransformToolService {
    ...
    private rotateFixed(originalAngle: number) {
        ...
        let angle = (this.canvas.getMainImage().angle + originalAngle) % 360;

        let height = Math.abs(this.canvas.getMainImage().width*(Math.sin(angle*Math.PI/180)))+Math.abs(this.canvas.getMainImage().height*(Math.cos(angle*Math.PI/180))),
