...
export class GoogleFontsPanelService {
    ...
    /**
     * GoogleFontsPanelService Constructor.
     */
    constructor(
        ...
        this.customFonts = this.settings.get('pixie.tools.text.items', []);
    }

    public async init() {
        let fonts = this.customFonts.slice();

        if ( ! this.settings.get('pixie.tools.text.replaceDefault')) {
            fonts = fonts.concat(await this.getGoogleFonts());
        }

        this.paginator.setFonts(fonts);
    }
